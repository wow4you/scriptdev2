        ...
        switch (m_uiPhase)
        {
                ...
                if (m_bIsIntro)
                {
                    switch (m_uiIntroStep)
                    {
                        ...
                        case 5:
                            ...
                            {
                                pSif->setFaction(35);
                                m_sifGuid = pSif->GetObjectGuid();
                            }
                            ++m_uiIntroStep;
                            m_uiIntroTimer = 9000;
                            break;
                        case 7:
                            if (Creature* pSif = m_pInstance->instance->GetCreature(m_sifGuid))
            ...
            case PHASE_BALCONY:
            {
                ...
                if (m_uiPhase2Timer < uiDiff && m_bIsPhaseEnd)
                {
                    ...
                    if (m_bIsHardMode)
                    {
                        if (Creature* Sif = m_pInstance->instance->GetCreature(m_sifGuid))
                        {
                            Sif->setFaction(14);
            ...
            case PHASE_OUTRO:
            {
                switch (m_uiOutroStep)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 5:
                        DoScriptText(m_bIsHardMode ? SAY_OUTRO_HARD_1 : SAY_OUTRO_1, m_creature);
                        if (m_bIsHardMode)
                        {
                            if (Creature* pSif = m_pInstance->instance->GetCreature(m_sifGuid))
                                DoCast(pSif, SPELL_STORMHAMMER);
                        }
                    ...
                    case 7:
                        ...
                        break;
                    case 9:
                        DoScriptText(m_bIsHardMode ? SAY_OUTRO_HARD_2 : SAY_OUTRO_2, m_creature);
                        ++m_uiOutroStep;
                        m_uiOutroTimer = 13000;
                        break;
                    case 11:
                        DoScriptText(m_bIsHardMode ? SAY_OUTRO_HARD_3 : SAY_OUTRO_3, m_creature);
                        ++m_uiOutroStep;
...
struct MANGOS_DLL_DECL boss_runic_colossusAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
        ...
        {
            m_smashTargetGuid = pWho->GetObjectGuid();
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            if (Unit* pTarget = m_creature->GetMap()->GetUnit(m_smashTargetGuid))
