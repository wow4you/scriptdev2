...
struct MANGOS_DLL_DECL boss_vx001AI : public ScriptedAI
{
    ...
    void SetPhase()
    {
        ...
        m_creature->MonsterMoveWithSpeed(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ() +  3, 3.f);
    }
...
struct MANGOS_DLL_DECL boss_aerial_command_unitAI : public ScriptedAI
{
    ...
    void Aggro(Unit* who)
    {
        if (m_pInstance->GetData(TYPE_MIMIRON_PHASE) == PHASE_AERIAL)
        {
            ...
            m_creature->GetMotionMaster()->MoveIdle();
            SetCombatMovement(false);
            m_creature->MonsterMoveWithSpeed(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ() +  7, 3.f);
        }
    }
    ...
    void SetPhase()
    {
        ...
        m_creature->SetUInt32Value(UNIT_FIELD_BYTES_1, 0);
        SetCombatMovement(false);
        m_creature->MonsterMoveWithSpeed(m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ() + 6.5f, 3.f);
    }

    // get the boss down by the magnetic core
    void SetToGround()
    {
        m_creature->MonsterMoveWithSpeed(m_creature->GetPositionX(), m_creature->GetPositionY(), CENTER_Z, 3.f);
        m_bIsGrounded = true;
        // make boss land
        m_creature->SetUInt32Value(UNIT_FIELD_BYTES_0, 0);
        m_creature->SetUInt32Value(UNIT_FIELD_BYTES_1, 0);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        switch (m_pInstance->GetData(TYPE_MIMIRON_PHASE))
        {
                        ...
                        case 5:
                            if (Creature* pTank = m_pInstance->GetSingleCreatureFromStorage(NPC_LEVIATHAN_MK))
                            {
                                if (pTank->isAlive())
                                {
                                    ...
                                    m_tankGuid = pTank->GetObjectGuid();
                                }
                                else
                                    EnterEvadeMode();
                            }
                            ...
                            break;
                        case 7:
                            if (Creature* pTank = m_pInstance->instance->GetCreature(m_tankGuid))
                            {
                                if (m_bIsHardMode)
                                {
            ...
            case PHASE_TRANS_1:
            {
                ...
                if (m_uiPhaseDelayTimer < uiDiff && !m_bHasMoreHp)
                {
                    ...
                    if (Creature* pTorso = m_creature->SummonCreature(NPC_VX001, CENTER_X, CENTER_Y, CENTER_Z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000))
                    {
                        if (m_bIsHardMode)
                        {
                            ...
                            m_torsoGuid = pTorso->GetObjectGuid();
                        }
                        else
                        {
                ...
                {
                    if (Creature* pTorso = m_pInstance->instance->GetCreature(m_torsoGuid))
            ...
            case PHASE_TRANS_2:
            {
                if (m_uiPhaseDelayTimer < uiDiff && !m_bHasMoreHp)
                {
                    ...
                    if (Creature* pHead = m_creature->SummonCreature(NPC_AERIAL_UNIT, CENTER_X, CENTER_Y, CENTER_Z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000))
                    {
                        ...
                        m_headGuid = pHead->GetObjectGuid();
                    }
                }
                else m_uiPhaseDelayTimer -= uiDiff;
                ...
                if (m_uiHpCheckTimer <= uiDiff && m_bHasMoreHp)
                {
                    if (Creature* pHead = m_pInstance->instance->GetCreature(m_headGuid))
